...
Each `cargo rustc --crate-type` invocation *overrides* the declared crate-type for that build, producing only the requested artifact. Run once per artifact you need. The C header `sproink.h` is regenerated by `build.rs` whenever the `ffi` feature is enabled.
